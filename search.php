...
{ 
  ...
  border-left:1px solid #52A1D5;
  border-right:1px solid #52A1D5;
...
if(isset($_GET['search']))
{
  ...
  $sql = mysqli_query($conn,"SELECT * FROM `posts` p INNER JOIN `users` u  WHERE p.p_author = u.u_id AND( `p_post` LIKE '%$search%' OR `p_title` LIKE '%$search%' OR `p_category` LIKE '%$search%' OR `p_post` LIKE '%$search%' OR `u_name` LIKE '%$search%') ORDER BY `p_id` DESC LIMIT $start_f , $per_page ") or die("No Result Found");
...
<!-- start-->
        ...
        <?php echo "<h1 style='text-align:center; margin:200px;'>$output</h1>"; ?>
...
<?php
               ...
               $page_sql = mysqli_query($conn, "SELECT * FROM `posts` p INNER JOIN `users` u WHERE (p.p_author = u.u_id)  AND (`u_name` LIKE '%$_GET[search]%' OR `p_post` LIKE '%$_GET[search]%' OR `p_title` LIKE '%$_GET[search]%' OR `p_category` LIKE '%$_GET[search]%') ");
               ...
               {
                echo '<li '.($page==$i ? 'class="active"' : '').' ><a href="search.php?search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
               }
